...
def fmt_loading(text: str) -> str:
    """Formats status message text in unified Pith style."""
    return f"⋯ {text}"


async def send_typing_safe(context: ContextTypes.DEFAULT_TYPE, chat_id: int) -> None:
    """Safely sends typing action to chat."""
    try:
...
async def send_loading_placeholder(message, text: str):
    """Sends a status placeholder message with formatting."""
    try:
        return await safe_reply(message, fmt_loading(text))
    except Exception as e:
        logger.debug(f"Failed to send loading placeholder: {e}")
        return None


async def edit_loading_placeholder(msg, text: str) -> bool:
    """Edits an existing status placeholder message."""
    if not msg:
        return False
    try:
        ...
        logger.debug(f"Failed to edit loading placeholder: {e}")
        return False
...
async def pith_loading_loop(
    ...
    """
    Background loop for updating status messages.
    - Updates text by phases (PITH_LOADING_STATES)
    - Periodically sends typing action
    - Stops on stop_event
    """
    ...
    while not stop_event.is_set():
        ...
        if phase_index < len(PITH_LOADING_STATES):
            ...
            if elapsed >= trigger_at:
                await edit_loading_placeholder(
                    loading_msg,
                    PITH_LOADING_STATES[phase_index],
                )
                phase_index += 1
...
# === PITH SYSTEM MESSAGES ===


def _fmt_sys(text: str) -> str:
    """Formats system message with prefix if enabled."""
...
memory = get_memory()
planner = RuntimePlanner(memory_manager=memory, system_prompt=SYSTEM_PROMPT)
...
def format_response_with_prefix(text: str, strip_known_prefixes: bool = True) -> str:
    """
    Unified response formatter for Telegram.

    - If show_prefix=False → returns clean text.
    - If strip_known_prefixes=True → strips prefixes from config.
    - Adds prefix from config if enabled.
    """
    if not text:
        return text
...
async def safe_reply(message, text, reply_markup=None, retries: int = 3):
    """Sends message with retry on network errors."""
    for attempt in range(retries):
        try:
            ...
            if attempt == retries - 1:
                raise
            logger.warning(f"Reply failed (attempt {attempt+1}/{retries}): {e}")
            await asyncio.sleep(2**attempt)
    return None


async def safe_callback_reply(query, text, reply_markup=None, retries: int = 3):
    """Callback-safe reply helper for feedback acknowledgements."""
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt == retries - 1:
                raise
            logger.warning(
                f"Callback reply failed (attempt {attempt+1}/{retries}): {e}"
            )
            await asyncio.sleep(2**attempt)
    return None
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Catches all unhandled exceptions in handlers and logs them centrally.
    """
    logger.error("Unhandled exception in Telegram handler", exc_info=context.error)

    # Try to notify the user gently if possible
    try:
        ...
        logger.debug(f"Failed to send error notification to user: {e}")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"📩 {user_id} (task {task_id}): {text[:100]}")
    ...
    memory.save_episode(
        ...
        "user",
        text,
        metadata={"task_id": task_id, "channel": "telegram"},
    )

    # 3) status → executing
    task_service.update_status(task_id, TaskState.executing)

    # 4) loading loop before planner
    chat_id = update.effective_chat.id
    await send_typing_safe(context, chat_id)
    loading_msg = await send_loading_placeholder(
        update.message,
        PITH_LOADING_STATES[0],
    )
...
async def handle_feedback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    episode = memory.find_episode_by_task_id(user_id, task_id, role="assistant")
    if not episode:
        with contextlib.suppress(Exception):
    ...
    ack = (
        ...
        else "Feedback recorded: negative."
    )
    await safe_callback_reply(
        query,
        format_response_with_prefix(ack, strip_known_prefixes=False),
    )
...
async def search_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from core.action.tavily_tool import search, format_search_results

    try:
        data = search(query, search_depth=depth)
        reply = format_search_results(data)
        await safe_reply(
            update.message,
            format_response_with_prefix(
                reply,
...
def _validate_interface_config() -> bool:
    """Validates that interface config has expected structure."""
    ...
    if not isinstance(KNOWN_PREFIXES, list) or not all(
        isinstance(p, str) for p in KNOWN_PREFIXES
    ):
...
def build_application() -> Application:
    """Builds telegram Application with tuned HTTPXRequest."""
    ...
    # 🔴 ВАЖНО: глобальный обработчик ошибок
...
if __name__ == "__main__":
    ...
    logger.info(
        "Pith runtime initialized. Interface prefix: '%s', loading profile: '%s'",
